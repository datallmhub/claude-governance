...
- Always distinguish between blocking issues (must fix) and suggestions (optional improvement):
  - Blocking: `[blocking] This query is missing org_id filter — IDOR risk.`
  - Suggestion: `[nit] Could simplify with a stream here.`
...
- Always treat these changes as non-breaking (safe to add): new optional fields, new endpoints, new optional query parameters.
...
- Always notify consumers (internal or external) at least 2 sprints before a breaking change goes live.
- Always add a deprecation notice in the API response before removing a field:
...
- Always pin versions in `pom.xml` and `package.json`. No open ranges in production.
- Always review the changelog and license before adding a new dependency.
- Always prefer a well-maintained dependency with > 1 000 GitHub stars over a custom implementation.
- Always run `mvn dependency-check:check` and `npm audit` before each release.
...
3. Version is bumped in `pom.xml` / `package.json`.
